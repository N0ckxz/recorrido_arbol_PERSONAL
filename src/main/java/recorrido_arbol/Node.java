...
public class Node {
    int data = 0;
    List<Node> children; // Para el arreglo dinámico

    Node() { //Constructor por defecto 
        this.children = new LinkedList<>();
    }  
    
    Node(int x) { //Constructor principal
        this.data = x;
    ...
    Node search(int searchValue) {
        ...
        if(this.data == searchValue) { //Si el dato del nodo actual es el mismo que el dato ingresado, verdadero
            return this;
        } 
        ...
        for (int i = 0; i <children.size(); i++) {
            //Con ayuda de Gemini
            Node findInChild = children.get(i).search(searchValue);
